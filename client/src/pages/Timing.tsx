...
export default function Timing() {
  ...
  const [, routeParams] = useRoute("/project/:id/song");
  ...
  const saveTranscriptMutation = trpc.timing.replaceTranscript.useMutation({ onSuccess: () => persistentProject && utils.project.workspace.invalidate({ projectId }) });
  const saveBeatsMutation = trpc.timing.replaceBeats.useMutation({ onSuccess: () => persistentProject && utils.project.workspace.invalidate({ projectId }) });
  const beatCount = useMemo(() => beats.filter(Boolean).length, [beats]);

  useEffect(() => {
    if (!workspaceQuery.data) return;
    if (workspaceQuery.data.tokens.length) {
      const grouped = new Map<number, string[]>();
      workspaceQuery.data.tokens.forEach((token) => grouped.set(token.lineIndex, [...(grouped.get(token.lineIndex) ?? []), token.text]));
      setLines(Array.from(grouped.entries()).sort(([left], [right]) => left - right).map(([, tokens]) => tokens));
    }
    if (workspaceQuery.data.beats.length) setBeats(workspaceQuery.data.beats.map(() => true));
  }, [workspaceQuery.data]);
  ...
  const persistTranscript = () => {
    if (!persistentProject) {
      toast.message("Sign in to save timing across devices.");
      startLogin();
      return;
    }
    saveTranscriptMutation.mutate({ projectId, tokens: lines.flatMap((line, lineIndex) => line.map((text, tokenIndex) => ({ lineIndex, tokenIndex, text }))) });
    setLyricsConfirmed(true);
    toast.success("Lyric timing saved to your project.");
  };

  const persistBeats = () => {
    if (!persistentProject) {
      toast.message("Sign in to save beat maps across devices.");
      startLogin();
      return;
    }
    saveBeatsMutation.mutate({ projectId, beats: beats.map((active, index) => active ? { timestampMs: index * 750, strength: index % 4 === 0 ? 88 : 60 } : null).filter(Boolean) as Array<{ timestampMs: number; strength: number }> });
    setBeatsConfirmed(true);
    toast.success("Beat map saved to your project.");
  };

  return (
    <main className="timing-shell">
      <div className="ambient-field" aria-hidden="true" />
      <header className="top-rail"><a className="brand-lockup" href="/dashboard"><img src="/manus-storage/framelab-mark_27a43cfa.png" alt="" className="brand-mark" /><span>FRAME<span>LAB</span></span></a><nav className="top-nav"><a href="/dashboard">DASHBOARD</a><a href={persistentProject ? `/project/${projectId}/preview` : "/project/draft-04/preview"}>WORKSPACE</a><a className="is-current" href={persistentProject ? `/project/${projectId}/song` : "/project/draft-04/song"}>TIMING</a></nav><div className="top-actions"><button type="button" className="credit-chip"><Sparkles size={13} /> {persistentProject ? "SYNC" : "FREE"}</button><button type="button" className="avatar-button" onClick={() => isAuthenticated ? toast.message(`Signed in as ${user?.name ?? "FrameLab creator"}.`) : startLogin()}>{user?.name?.slice(0, 1).toUpperCase() ?? "B"}</button></div></header>
